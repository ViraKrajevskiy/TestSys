...
(function () {
   ...
   * Возвращает true, если токен обновился — тогда исходный запрос
    ...
    try {
      ...
      if (!resp || !resp.ok || resp.status_code >= 400) {
        App.logWarn && App.logWarn("Auth",
          `Refresh не удался: ${resp && resp.status_code ? resp.status_code : "нет ответа"}. ` +
          "Скорее всего, refresh-токен тоже истёк — нужен новый вход.");
        return false;
      }

      let data;
      try { data = JSON.parse(resp.text || "null"); }
      catch { App.logWarn && App.logWarn("Auth", "Refresh вернул не JSON"); return false; }
